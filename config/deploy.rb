...
set :whenever_command, lambda {
  "pwd && #{fetch(:rvm_command_prefix)} bundle exec whenever" #--set 'script_env=#{fetch(:deploy_name)}&path=#{deploy_to}/current/gsas_sync_main.rb&rvm_prefix=#{fetch(:rvm_command_prefix)}'"
}
